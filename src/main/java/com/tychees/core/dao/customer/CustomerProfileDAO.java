...
public class CustomerProfileDAO 
	...
	{
		return CustomerProfileBean.class.getSimpleName();
	}
  
	public CustomerProfileBean getBeanById(String id) 
		throws DAOException
	{  
		try 
		{
			...
			List<CustomerProfileBean> list = query.list();
			if (list.size() > 0)
				return list.get(0);  
			else
				return null;
	...
	public List<CustomerProfileBean> getBeansByCustomerId(String customerId) 
		throws DAOException
	{  
		try 
		{
