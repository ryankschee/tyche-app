...
public class EmployeeAddressDAO 
	...
	{
		return EmployeeAddressBean.class.getSimpleName();
	}
  
	public EmployeeAddressBean getBeanById(String id) 
		throws DAOException
	{  
		try 
		{
			...
			List<EmployeeAddressBean> list = query.list();
			if (list.size() > 0)
				return list.get(0);  
			else
				return null;
	...
	public List<EmployeeAddressBean> getBeansByEmployeeId(String employeeId) 
		throws DAOException
	{  
		try 
		{
