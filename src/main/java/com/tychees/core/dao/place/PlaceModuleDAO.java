...
@Repository  
public class PlaceModuleDAO 
	extends AbstractDAO {  
  
	@Override
	public String getBeanName() 
	{
		return PlaceModuleBean.class.getSimpleName();
	}
	
	public PlaceModuleBean getBeanById(String id) 
		throws DAOException
	{  
		try 
		{
			...
			List<PlaceModuleBean> list = query.list();
			if (list.size() > 0)
				return list.get(0);  
			else
				return null;
	...
	public List<PlaceModuleBean> getBeansByPlaceId(String placeId) 
		throws DAOException
	{  
		try 
		{
			...
			List<PlaceModuleBean> list = query.list();		
			if (list.size() == 0) 
				return new ArrayList<PlaceModuleBean>();  
			else 
				return list;
		} 
		catch (Exception e) 
