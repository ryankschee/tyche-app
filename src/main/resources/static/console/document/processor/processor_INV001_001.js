 ...
 */
function processDocument_INV001_001_getPDF(documentObj, placeDocumentObj, userObj, billingObj) {
	// 0. Get objects
	...
	*/
	...
	var l_tinDate = tycheesDateTime_getFormattedDate_ddMMyyyy(billingObj.invoiceDate);
	/*
	var propObj_tinDate = documentCommon_getPropObjectByKey(placeDocumentObj, '$_TIN_DATE_$');
	if (propObj_tinDate == null) {
		l_tinDate = tycheesDateTime_getFormattedDate_ddMMyyyy(billingObj.invoiceDate);
	} else {
		if ($.trim(propObj_tinDate.userInput) == '')
			l_tinDate = tycheesDateTime_getFormattedDate_ddMMyyyy(billingObj.invoiceDate);
		else 
		if ($.trim(propObj_tinDate.userInput) == 'TODAY')
			l_tinDate = tycheesDateTime_getFormattedDate_ddMMyyyy(new Date().getTime());
		else
			l_tinDate = $.trim(propObj_tinDate.userInput);
	}*/
	...
	pdfDoc.setFontSize(14);
	pdfDoc.text('Billing No', 565, 110, 'right');
	pdfDoc.setFontSize(16);
	pdfDoc.text(l_billingNo, 565, 130, 'right');
	// Office Phone
	...
	pdfDoc.setTextColor(0, 0, 0);
	pdfDoc.setFontType('bold');
	pdfDoc.text('PAYMENT DATE', 30, billingTable_yOffset);
	pdfDoc.text('METHOD', 350, billingTable_yOffset, 'right');
	pdfDoc.text('AMOUNT (' + c_currency.toUpperCase() + ')', 565, billingTable_yOffset, 'right');
	...
	if (billingObj.paymentList.length == 0) {
		...
		billingTable_yOffset += 15;
		pdfDoc.text('- NO PAYMENT RECEIVED -', 30, billingTable_yOffset);
	}
	else {
		pdfDoc.setFontSize(12);
		pdfDoc.setTextColor(0, 0, 0);
		...
		$.each(billingObj.paymentList, function(i, paymentObj) {
			...
			pdfDoc.text(tycheesDateTime_getFormattedDate_ddMMyyyy(paymentObj.paymentDate), 30, billingTable_yOffset);
			pdfDoc.text(paymentObj.method + ' (' + paymentObj.methodDesc + ')', 350, billingTable_yOffset, 'right');
			pdfDoc.text(Number(paymentObj.payingAmount).toFixed(2), 565, billingTable_yOffset, 'right');
			// Line Break
			billingTable_yOffset += 5;
			pdfDoc.setLineWidth(0.05);
			pdfDoc.line(30, billingTable_yOffset, 565, billingTable_yOffset);
	...
	*/
	return pdfDoc;
} // .end of processDocument_INV001_001_getPDF
