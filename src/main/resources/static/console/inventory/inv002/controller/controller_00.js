/*******************************************************************************
 * Generate unique ID for transaction, based on given type.
 * 
 * @pi_transactionType	transaction type
 ...
 */
function getNextTransactionId(pi_transactionType) {
	if (pi_transactionType == InventoryConstants.TRANSACTION_TYPE_ADJUSTMENT) {
		return 'TRX-' + $.now();
	}
	else
	if (pi_transactionType == InventoryConstants.TRANSACTION_TYPE_PURCHASE) {
		return 'TRX-' + $.now();
	}
	else
	if (pi_transactionType == InventoryConstants.TRANSACTION_TYPE_TRANSFER) {
		return 'TRX-' + $.now();
	}
	else
	if (pi_transactionType == InventoryConstants.TRANSACTION_TYPE_LOST) {
		return 'TRX-' + $.now();
	}
	else
	if (pi_transactionType == InventoryConstants.TRANSACTION_TYPE_BILLING) {
		return 'TRX-' + $.now();
	}
	else {
		return false;
	}
} // .end of getNextTransactionId
