 ...
 */
function processDocument_FOD001_001_getPDF(documentObj, placeDocumentObj, userObj, billingBucketObj) {
	// 0. Get objects
	var placeObj = tycheesCommon_getCurrentPlaceObject();

	// 0.5. Get system-settings input objects
	var foodOrderLabel = settingsCommon_getSettingsValue(
							ModuleConstants.MODULE_BILLING, 
							ModuleConstants.MODULE_BILLING_SUB_BACKOFFICE, 
							SettingsConstants.SETTINGS_POS_KEY_FOOD_ORDER_LABEL);
	...
	// 2.0 Prepare PDF doc
	var pageWidth = 200;
	var pageHeight = processDocument_FOD001_001_getPageHeight(documentObj, placeDocumentObj, userObj, billingBucketObj);	
	...
	pdfDoc.setProperties({
		title: 'Food Order for ' + billingBucketObj.bucketName,
		subject: 'Food Order',
	...
	// Y Axis
	var yOffset = 30;
	pdfDoc.text('FOOD ORDER', documentCommon_getXOffset(pdfDoc, 'FOOD ORDER'), yOffset);
	...
	$.each(billingBucketObj.lineList, function(i, lineObj) {
		...
		if ($.inArray(foodOrderLabel, labelList) == -1)
			return true;
 ...
 * @returns {Object} jsPDF object
 */
function processDocument_FOD001_001_getPageHeight(documentObj, placeDocumentObj, userObj, billingBucketObj) {
