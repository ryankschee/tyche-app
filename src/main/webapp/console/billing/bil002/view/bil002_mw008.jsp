...
function mw008_open_postHandler(successFlag, responseObj, params)
{
	...
	if (mw008_itemObj != null && mw008_billingLineObj != null)
	{
		...
		$('#mw008_defaultPrice').val(tycheesUtil_getSafeNumber(mw008_billingLineObj.defaultPrice, 2));
		
		if (mw008_billingLineObj.unitPriceSource=='default')
		{
			$('#mw008_setDefaultPrice').iCheck('check');
			$('#mw008_defaultPrice').val(tycheesUtil_getSafeNumber(mw008_billingLineObj.subUnitPrice, 2));
		...
		$.each(mw008_itemObj.componentList, function(i, componentObj){
			...
			if (componentObj.selectable==true)
			{
		        ...
		        var component_categoryObj = Cached_getCategoryById(componentObj.categoryId, bil002_categoryList);	
...
function mw008_processSelectableComponents(rootItemObj, rootComponentObj, itemObj, componentCount)
{
	$.each(itemObj.componentList, function(i, componentObj){
        ...
        var itemList = Cached_getItemListByComponent(componentObj, bil002_itemList);
...
function mw008_getSelectableComponentHtml(componentObj, billingLineObj, isNewItem)
{
	...
	var quantityVal = componentObj.defaultQuantity;
	var taxVal = 0;
	var discountVal = 0;
	var selectorId = tycheesText_replaceAll(componentObj.id, '-', '_');
    ...
    var quantityInput = '<input type="number" id="quantity-' + componentObj.id + '-' + billingLineObj.itemId + '" step="any" min="0.001" required value="' + quantityVal + '" style="width:60px;text-align:center;" onchange="mw008_onQuantityChanged(\'' + componentObj.id + '\',\'' + billingLineObj.itemId + '\');">';
    if (componentObj.allowDefaultQuantityOnly==true)
    	quantityInput = '<input type="number" id="quantity-' + componentObj.id + '-' + billingLineObj.itemId + '" value="' + quantityVal + '" style="width:60px;text-align:center;" disabled>';
	
	var taxOrDiscountInput = '';
	if (billingLineObj.itemType==InventoryConstants.ITEM_TYPE_TAX)
	{
		taxOrDiscountInput = '<input type="number" id="taxOrDiscount-' + componentObj.id + '-' + billingLineObj.itemId + '" value="' + taxVal + '" style="width:60px;text-align:center;" disabled>';
	}
	if (billingLineObj.itemType==InventoryConstants.ITEM_TYPE_DISCOUNT)
	{
		taxOrDiscountInput = '<input type="number" id="taxOrDiscount-' + componentObj.id + '-' + billingLineObj.itemId + '" value="' + discountVal + '" style="width:60px;text-align:center;" disabled>';
	}
	
	var unitPriceVal = billingLineObj.subUnitPrice;
	var unitPriceInput = '<input type="number" id="unitprice-' + componentObj.id + '-' + billingLineObj.itemId + '" step="any" min="0.01" required value="' + unitPriceVal + '" style="width:80px;text-align:center;" onchange="mw008_onQuantityChanged(\'' + componentObj.id + '\',\'' + billingLineObj.itemId + '\');">';
    if (componentObj.allowDefaultQuantityOnly==true)
    	unitPriceInput = '<input type="number" id="unitprice-' + componentObj.id + '-' + billingLineObj.itemId + '" value="' + unitPriceVal + '" style="width:80px;text-align:center;" disabled>';
	
	var totalPriceVal = Number(quantityVal) * Number(unitPriceVal);
	var totalPriceInput = '<input type="number" id="totalPrice-' + componentObj.id + '-' + billingLineObj.itemId + '" value="' + tycheesUtil_getSafeNumber(totalPriceVal, 2) + '" style="width:80px;text-align:center;" disabled>';
	
	//var optionalSelectionCount = mw008_countOptionalSelectionsInItem(itemObj);
	//var selectableFlag = '';
	//if (componentObj.selectable==true && optionalSelectionCount > 0)
	//	selectableFlag = '<small class="label label-warning" style="color:#FF0000;"><i class="fa fa-hand-o-left" aria-hidden="true"></i> Click Here</small>';
	...
	var htmlLines = '';
    if (componentObj.selectable==true)
    	htmlLines += '<li class="selectable ' + newItemClass + '" id="selectable-component-item-' + componentObj.id + '-' + billingLineObj.itemId + '">';
    else
    	htmlLines += '<li id="selectable-component-item-' + componentObj.id + '-' + billingLineObj.itemId + '">';
    ...
    htmlLines += '<input type="hidden" id="componentId" value="' + componentObj.id + '">';
    ...
    if (billingLineObj != null)
    {
    	...
    	else
    	{
    	    ...
    	    htmlLines += '<i class="fa fa-times" aria-hidden="true" style="color:#FF0000; cursor: pointer;" onclick="mw008_removeOptionalItem(\'' + componentObj.id + '\',\'' + billingLineObj.itemId + '\',\'' + billingLineObj.id +  '\');"></i> ';
    	    htmlLines += billingLineObj.itemName;
    	    htmlLines += '</span>';
    	}
    }
   	...
   	{
	    htmlLines += '<span class="m-l-xs" style="font-size:16px;">';   
	    htmlLines += '<i class="fa fa-times" aria-hidden="true" style="color:#FF0000; cursor: pointer;" onclick="mw008_removeOptionalItem(\'' + componentObj.id + '\',\'' + billingLineObj.itemId + '\');"></i> ';
	    htmlLines += billingLineObj.itemName;
	    htmlLines += '</span>';
   	}    
